...
export function InstructorNoticesPage() {
  ...
  return <StaffShell account={account} title="Learner notices"><div className="notice-layout"><section className="notice-composer"><p className="eyebrow">Course communication</p><h2>Say only what helps.</h2><p>Notices stay connected to the selected course unit so learners receive only what is useful to their revision.</p><form onSubmit={submit}><label>Course unit<select value={courseSpaceId} onChange={(event) => setCourseSpaceId(event.target.value)}>{spaces.map((course) => <option key={course.id} value={course.id}>{course.title}</option>)}</select></label><label>Notice title<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Message<textarea rows={5} value={body} onChange={(event) => setBody(event.target.value)} /></label><footer>{sent && <span className="save-message"><Check size={15} /> Notice recorded.</span>}<button type="submit" className="primary-action">Send notice <Send size={16} /></button></footer></form></section><section className="notice-history"><div className="section-heading"><div><p className="eyebrow">Recorded notices</p><h2>Recent communication</h2></div><FileText size={19} /></div>{notices.map((notice) => <article key={notice.id}><span className="notice-icon"><MessageSquareText size={17} /></span><div><strong>{notice.title}</strong><p>{notice.body}</p><small>{notice.createdAt} · {notice.status}</small></div></article>)}</section></div></StaffShell>
}
