...
export function PlannerPage() {
  ...
  const chooseEntryPoint = (termId: string) => {
    updateActivePlan((current) => ({ ...current, entryTermId: termId, activeTermId: termId }))
...
function EntryScreen({ pathwayId, selectedTermId, onChoose, onBack }: { pathwayId: PathwayId; selectedTermId: string; onChoose: (termId: string) => void; onBack: () => void }) {
  ...
  return <section className="degree-stage"><CompletedStep number="1" label={pathway.credential} action="Change certificate" onClick={onBack} /><StepHeader number="2" title="Set your entry point" copy="Choose the semester you are joining. Semester 1 runs February–June; Semester 2 runs July–December. If you join later, record school-cleared semesters in My Progress before registering a later semester." /><div className="degree-choice-list degree-choice-list--terms">{pathway.terms.map((term) => <article key={term.id} className={term.id === selectedTermId ? 'is-selected' : ''}><div><strong>{term.label}</strong><small>{term.period} · {getTermUnits(pathway, term.id).length} individual revision courses</small></div><button className="degree-button degree-button--outline" onClick={() => onChoose(term.id)}>Choose</button></article>)}</div><button className="degree-back" onClick={onBack}><ArrowLeft size={15} /> Back</button><DegreeFooter /></section>
}
...
function PlanScreen({ pathwayId, selectedTermId, onChangeCertificate, onOpenRegistration, onOpenProgress, onChooseTerm, onOpenCalendar }: { pathwayId: PathwayId; selectedTermId: string; onChangeCertificate: () => void; onOpenRegistration: () => void; onOpenProgress: () => void; onChooseTerm: (termId: string) => void; onOpenCalendar: () => void }) {
  ...
  const readiness = getTermReadiness(pathway, plan, selectedTerm.id)
  return <section className="degree-plan"><PlannerNav active="plan" onPlan={() => undefined} onRegistration={onOpenRegistration} onProgress={onOpenProgress} /><header className="degree-plan__header"><div><span className="eyebrow">My plan</span><h1>{pathway.credential}</h1><p>DeepFocus keeps this revision pathway local to this browser. It is not an official school record.</p></div><button className="degree-button degree-button--outline" onClick={onChangeCertificate}>Change certificate</button></header><section className="degree-plan__status"><span><Check size={14} /> Entry point: {pathway.terms.find((term) => term.id === plan.entryTermId)?.label} · {pathway.terms.find((term) => term.id === plan.entryTermId)?.period}</span><span><Clock3 size={14} /> {plan.sessionsPerWeek} sessions each week</span><span><CircleCheckBig size={14} /> {plan.registeredUnitIds.length} course{plan.registeredUnitIds.length === 1 ? '' : 's'} registered</span></section><div className="degree-plan__workspace"><TermList pathwayId={pathwayId} selectedTermId={selectedTerm.id} onChoose={onChooseTerm} /><section className="degree-term-detail"><header><div><span className="eyebrow">Selected semester</span><h2>{selectedTerm.label}</h2></div><span>{selectedTerm.period} · {selectedUnits.length} courses</span></header>{!readiness.ready && <div className="degree-gate"><Lock size={17} /><div><strong>Earlier-semester clearance needed</strong><p>Open My Progress to record the semesters already cleared by your school before registering this semester.</p></div><button onClick={onOpenProgress}>My Progress <ChevronRight size={15} /></button></div>}<div className="degree-course-list">{selectedUnits.map((unit) => <article key={unit.id}><span>{unit.code}</span><div><strong>{unit.title}</strong><small>{plan.registeredUnitIds.includes(unit.id) ? 'Registered for DeepFocus revision' : 'Not registered'}</small></div>{unit.revisionMapIds?.length ? <span className="degree-course-list__maps">{unit.revisionMapIds.length} map{unit.revisionMapIds.length === 1 ? '' : 's'}</span> : <span className="degree-course-list__maps is-quiet">No map yet</span>}</article>)}</div><footer><p>{readiness.ready ? 'Review this semester, then register only the courses you want to appear on your dashboard.' : 'Clearance is a personal planning record; it is not an official prerequisite decision.'}</p><button className="degree-button degree-button--primary" onClick={onOpenRegistration} disabled={!readiness.ready}>Review registration <ArrowRight size={15} /></button></footer></section></div><section className="degree-calendar-callout"><div><strong>Need to place a date?</strong><p>Use the separate focus calendar after you have shaped the pathway.</p></div><button onClick={onOpenCalendar}>Open calendar <ChevronRight size={15} /></button></section><DegreeFooter /></section>
}
...
function ProgressScreen({ pathwayId, onBack, onToggleClearance }: { pathwayId: PathwayId; onBack: () => void; onToggleClearance: (termId: string) => void }) {
  const pathway = getStudyPathway(pathwayId)
  const plan = getPathwayPlan(loadPlannerState(), pathwayId)
  return <section className="degree-plan"><PlannerNav active="progress" onPlan={onBack} onRegistration={onBack} onProgress={() => undefined} /><header className="degree-registration-header"><button className="degree-back" onClick={onBack}><ArrowLeft size={15} /> My Plan</button><div><span className="eyebrow">My Progress</span><h1>Record prior clearance.</h1><p>For learners joining later, record only semesters that your school has already completed, credited, or formally cleared.</p></div></header><section className="degree-progress"><div className="degree-progress__notice"><CircleCheckBig size={20} /><p>These controls only save a private DeepFocus planning record. They do not award credit or change an official result.</p></div>{pathway.terms.map((term) => <article key={term.id}><div><strong>{term.label}</strong><small>{term.period} · {getTermUnits(pathway, term.id).length} individual revision courses</small></div><button className={plan.clearedTermIds.includes(term.id) ? 'is-cleared' : ''} onClick={() => onToggleClearance(term.id)}>{plan.clearedTermIds.includes(term.id) ? <><Check size={15} /> Cleared</> : 'Record clearance'}</button></article>)}<a href={pathway.source.url} target="_blank" rel="noreferrer">Review curriculum reference <ExternalLink size={14} /></a></section><DegreeFooter /></section>
}
