...
function EntryScreen({ pathwayId, selectedTermId, onChoose, onBack }: { pathwayId: PathwayId; selectedTermId: string; onChoose: (termId: string) => void; onBack: () => void }) {
  ...
  return <section className="degree-stage"><CompletedStep number="1" label={pathway.credential} action="Change certificate" onClick={onBack} /><StepHeader number="2" title="Set your entry point" copy="Choose the term you are joining. Semester 1 runs February–June; Semester 2 runs July–December. If you join later, record school-cleared terms in My Progress before registering a later term." /><div className="degree-choice-list degree-choice-list--terms">{pathway.terms.map((term) => <article key={term.id} className={term.id === selectedTermId ? 'is-selected' : ''}><div><strong>{term.label}</strong><small>{term.period} · {getTermUnits(pathway, term.id).length} individual course units</small></div><button className="degree-button degree-button--outline" onClick={() => onChoose(term.id)}>Choose</button></article>)}</div><button className="degree-back" onClick={onBack}><ArrowLeft size={15} /> Back</button><DegreeFooter /></section>
}
...
function ProgressScreen({ pathwayId, onBack, onToggleClearance }: { pathwayId: PathwayId; onBack: () => void; onToggleClearance: (termId: string) => void }) {
  const pathway = getStudyPathway(pathwayId)
  const plan = getPathwayPlan(loadPlannerState(), pathwayId)
  return <section className="degree-plan"><PlannerNav active="progress" onPlan={onBack} onRegistration={onBack} onProgress={() => undefined} /><header className="degree-registration-header"><button className="degree-back" onClick={onBack}><ArrowLeft size={15} /> My Plan</button><div><span className="eyebrow">My Progress</span><h1>Record prior clearance.</h1><p>For learners joining later, record only terms that your school has already completed, credited, or formally cleared.</p></div></header><section className="degree-progress"><div className="degree-progress__notice"><CircleCheckBig size={20} /><p>These controls only save a private DeepFocus planning record. They do not award credit or change an official result.</p></div>{pathway.terms.map((term) => <article key={term.id}><div><strong>{term.label}</strong><small>{term.period} · {getTermUnits(pathway, term.id).length} individual course units</small></div><button className={plan.clearedTermIds.includes(term.id) ? 'is-cleared' : ''} onClick={() => onToggleClearance(term.id)}>{plan.clearedTermIds.includes(term.id) ? <><Check size={15} /> Cleared</> : 'Record clearance'}</button></article>)}<a href={pathway.source.url} target="_blank" rel="noreferrer">Review curriculum reference <ExternalLink size={14} /></a></section><DegreeFooter /></section>
}
