...
DeepFocus should model every purchasable study term as a distinct academic product:
...
Each product should grant access to a known set of individual course units and their revision maps. A learner can buy only one term, then later buy the next term; the application should never infer paid access from a planner choice.
...
4. Store an entitlement with: account ID, programme, academic year, term ID, purchased course-unit IDs, payment transaction ID, status, start date, end date, and refund/revocation history.
...
6. The dashboard calls `GET /me/entitlements`; it renders only units in an active entitlement. The planner may propose a future term, but only the API can mark it accessible.
...
The planner should still let a learner choose any entry term. In production, distinguish the learner's own study preference from verified academic clearance:

1. The learner selects their pathway and the term they want to join.
2. Earlier terms show **clearance pending** rather than an editable “cleared” switch.
...
5. The API then permits purchase or activation of the later-term product. If school rules require an individual course prerequisite, store that rule against that course unit rather than assuming that every earlier term is equivalent.
...
- **No active entitlement:** DeepFocus shows the branded first-plan screen and one clear action: choose a pathway and term.
- **Active entitlement:** the dashboard shows only the learner's current term, its paid course units, one study rhythm control, and the focus calendar.
- **Future or expired term:** show the term in the planner with a clear status and renewal/purchase action; do not show its protected maps.
- **Multi-term learner:** let the learner change the current dashboard term from the planner, while preserving a clean single-term dashboard.

## Delivery order

1. Freeze the course catalogue and term-product definitions with the academic owner.
